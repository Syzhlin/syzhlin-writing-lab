...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { text, mode, bookTitle, chapterTitle } = body;

    if (!text || typeof text !== "string") {
      res.writeHead(400, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ error: "글을 입력해 주세요." }));
    }
    ...
    if (!ai) {
      ...
      return res.end(JSON.stringify(fallbackText));
    }
    ...
    console.error("Vercel revise error:", error);
    const body = await getRequestBody(req);
    const textToUse = body.text || "";
    const modeToUse = body.mode || "";
    const fallbackText = getFallbackRevision(textToUse, modeToUse);

    let errorDetail = "네트워크 상태에 따라 내장 서선 정착 엔진이 기동되었습니다.";
    ...
    return res.end(JSON.stringify({
      ...fallbackText,
      reason: `[실시간 AI 상태에 따라 내장 엔진 지원 가동] ${fallbackText.reason} (${errorDetail})`
    }));
  }
}
